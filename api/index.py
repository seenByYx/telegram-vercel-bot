...
if not BOT_TOKEN:
    raise ValueError("❌ BOT_TOKEN is missing! Set it in Vercel’s environment variables.")
...
async def handle_user_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_id = msg.chat.id
    user_ids.add(user_id)
    forwarded = await context.bot.forward_message(chat_id=ADMIN_CHAT_ID, from_chat_id=user_id, message_id=msg.message_id)
...
bot_app.add_handler(MessageHandler(filters.Chat(ADMIN_CHAT_ID), handle_admin_reply))
bot_app.add_handler(MessageHandler(~filters.Chat(ADMIN_CHAT_ID), handle_user_message))
...
def webhook():
    try:
        ...
        bot_app.process_update(update)  # ✅ Correct way for webhooks on Vercel
        return "ok", 200
    except Exception as e:
        return f"Error: {e}", 500

# === WEBHOOK SETUP HELPER (optional manual use) ===
async def set_webhook():
    vercel_url = os.getenv("VERCEL_URL")
    if not vercel_url:
        print("⚠️ Missing VERCEL_URL env variable.")
        return
    url = f"https://{vercel_url}/{BOT_TOKEN}"
